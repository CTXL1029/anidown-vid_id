...
def lists_vids(ep_start, ep_end):
    ...
    print()
    for i in range(0, len(eps)):
        vid_id = str(input(f"{line.BOLD}{line.LIGHT_ORANGE}Nhập vid_id của tập {eps[i]}: {line.RESET}"))
...
def start():
    print(f"{line.BOLD}{line.CYAN}Trình Tải Anime Từ Server Abyss / HydraX | Tải Trọn Mùa 1 Bộ Anime")
    name = str(input(f"{line.BOLD}{line.LIGHT_ORANGE}Nhập tên của bộ anime cần tải: {line.RESET}"))
    ep_start, ep_end = map(int, input(f"{line.BOLD}{line.LIGHT_ORANGE}Nhập khoảng tập của bộ anime cần tải: {line.RESET}").split(" - "))
    seasons = str(input(f"{line.BOLD}{line.LIGHT_ORANGE}Nhập phần của bộ anime cần tải (Nhấn Enter để bỏ qua): {line.RESET}"))
